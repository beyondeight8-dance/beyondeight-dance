(async function () {
  ...
  const placeholder = (title, editSection) => `<section class="owner-dashboard-section"><header><div><p class="eyebrow">Website manager</p><h1>${title}</h1><p>Manage this content in your shared website editor.</p></div><a class="primary-button" href="/${encodeURIComponent(business.slug)}?owner=1&edit=${editSection}">Open ${title}</a></header></section>`;
  ...
  const render = () => { const views = { overview, classes: classView, registrations: registrationView, website: websiteView, settings: settingsView, instructors: () => comingSoon("Instructors", "Add co-teachers and let visitors see who's running each class."), reviews: () => comingSoon("Reviews", "Collect and showcase student reviews on your public site."), social: () => placeholder("Social", "social"), analytics: () => comingSoon("Analytics", "Visitor traffic and booking insights for your website.") }; root.innerHTML = `<div class="owner-shell">${nav()}${views[activeView]()}</div>`; bind(); };
