(async function () {
  ...
  const overview = () => {
    ...
    const upcomingLive = live.filter((item) => item.date && item.date >= today);
    ...
    const upcomingCapacity = upcomingLive.reduce((sum, item) => sum + (Number(item.capacity) || 0), 0);
    const upcomingRegistered = registrations.filter((entry) => upcomingLive.some((item) => item.id === entry.class_id)).length;
    const fillRate = upcomingCapacity ? Math.round((upcomingRegistered / upcomingCapacity) * 100) : null;
    ...
    const snapshotMarkup = `<div><span>Live Classes</span><strong>${live.length}</strong><small>on your website</small></div><div><span>Total Registrations</span><strong>${registrations.length}</strong><small>all time</small></div><div><span>Upcoming Classes</span><strong>${upcomingLive.length}</strong><small>${fillRate !== null ? `${fillRate}% fill rate` : "no capacity set"}</small></div><div${pendingCount ? " class=\"is-attention\"" : ""}><span>Pending Payments</span><strong>${pendingCount}</strong><small>awaiting confirmation</small></div>`;
    const activityMarkup = activity.length ? activity.map((item) => `<article><span class="owner-activity-mark" aria-hidden="true">${(item.payment_status || "").includes("paid") ? "$" : "+"}</span><p><strong>${esc(item.student_name || "A student")}</strong> registered for ${esc(item.class_snapshot?.title || "a class")}</p><small>${esc((item.payment_status || "registered").replaceAll("_", " "))}</small></article>`).join("") : `<p class="owner-empty-compact">No recent activity. New registrations will appear here.</p>`;
    return `<section class="owner-welcome"><div><h1>${greeting}, ${esc(name)}.</h1><p>Here's what's happening with your dance business.</p></div><button class="primary-button owner-create-class" data-add-class><span aria-hidden="true">+</span> Create New Class</button></section>${dirty() ? `<section class="owner-draft-banner"><div><strong>Unpublished Changes</strong><span>Your draft is private until you publish.</span></div><button data-publish>Publish Changes</button></section>` : `<p class="owner-published-state">Published and up to date</p>`}<div class="owner-overview-grid"><div class="owner-overview-main"><section class="owner-overview-section owner-overview-primary"><header><h2>Upcoming Classes</h2><button type="button" data-view="classes">View All →</button></header><div>${upcomingMarkup}</div></section><section class="owner-overview-section"><header><h2>Business Snapshot</h2></header><div class="owner-snapshot">${snapshotMarkup}</div></section><section class="owner-overview-section owner-activity"><header><h2>Recent Activity</h2><button type="button" data-view="registrations">View All →</button></header><div>${activityMarkup}</div></section></div><aside class="owner-overview-rail"><section class="owner-site-status"><img src="${esc(imageUrl(draftState.heroImage || "assets/starter-hero-dance.jpg"))}" alt=""><div><span class="owner-site-badge${dirty() ? " is-draft" : ""}">${dirty() ? "Draft" : "Live"}</span><a class="owner-site-url" href="/${encodeURIComponent(business.slug)}" target="_blank">beyond8dance.com/${esc(business.slug)}</a><a class="owner-site-view" href="/${encodeURIComponent(business.slug)}" target="_blank">View Website →</a></div></section><section class="owner-rail-actions"><h2>Quick Actions</h2><button type="button" data-add-class>+ Create New Class</button><button type="button" data-view="registrations">View Registrations</button><button type="button" data-view="website">Edit My Website</button><a href="/${encodeURIComponent(business.slug)}" target="_blank">Share My Link ↗</a></section></aside></div>`;
  };
