(async function () {
  ...
  const classView = () => `<section class="owner-dashboard-section"><header><div><p class="eyebrow">Schedule</p><h1>Classes</h1><p>Create, publish, and manage classes without leaving your dashboard.</p></div><button class="primary-button" data-add-class>+ Add Class</button></header><div class="owner-class-list">${classes().length ? classes().map(row).join("") : `<div class="owner-empty-state"><strong>Add your first class.</strong><p>Publish when you are ready for bookings.</p><button class="primary-button" data-add-class>Add a Class</button></div>`}</div></section>`;
  const registrationView = () => `<section class="owner-dashboard-section"><header><div><p class="eyebrow">Students</p><h1>Registrations</h1><p>Bookings from your published website.</p></div></header><div class="owner-registration-list">${registrations.length ? registrations.map((item) => `<article><div><strong>${esc(item.student_name)}</strong><span>${esc(item.student_email)} • ${esc(item.student_phone)}</span></div><div><strong>${esc(item.class_snapshot?.title || "Class booking")}</strong><span>${esc(item.class_snapshot?.date || "")} ${esc(item.class_snapshot?.time || "")}</span></div><span class="owner-status is-draft">${esc(String(item.payment_status || "pending").replaceAll("_", " "))}</span></article>`).join("") : `<div class="owner-empty-state"><strong>No registrations yet.</strong><p>Bookings appear here after a visitor completes registration.</p></div>`}</div></section>`;
  const websiteView = () => `<section class="owner-dashboard-section"><header><div><p class="eyebrow">Website</p><h1>Website content</h1><p>Edit the same shared website that visitors see.</p></div><a class="primary-button" href="/${encodeURIComponent(business.slug)}?owner=1&edit=hero">Open Website Editor</a></header></section>`;
  ...
  const placeholder = (title) => `<section class="owner-dashboard-section"><header><div><p class="eyebrow">Website manager</p><h1>${title}</h1><p>Manage this content in your shared website editor.</p></div><a class="primary-button" href="/${encodeURIComponent(business.slug)}?owner=1&edit=${title.toLowerCase()}">Open ${title}</a></header></section>`;
