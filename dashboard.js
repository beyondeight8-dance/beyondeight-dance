(async function () {
  ...
  const sourceState = (mode) => {
    ...
    const selected = mode === "published" ? website.published_content : website.draft_content;
    return clone(selected && Object.keys(selected).length ? selected : website.published_content || bundle.settings?.generated_content || {});
  };
  ...
  const registrationCount = (item) => {
    const capacity = Number.parseInt(item.capacity, 10);
    const spots = Number.parseInt(item.spots, 10);
    return Number.isFinite(capacity) && Number.isFinite(spots) ? Math.max(0, capacity - spots) : null;
  };
  const classRow = (item, index) => {
    const registered = registrationCount(item);
    const capacity = Number.parseInt(item.capacity, 10);
    return `<article class="owner-class-row">
      <img src="${esc(imageUrl(item.image))}" alt="" loading="lazy">
      <div><strong>${esc(item.title || "Untitled class")}</strong><span>${esc(classDate(item))}</span><small>${esc(item.location || "Location coming soon")} • ${esc(item.level || "Open level")}</small></div>
      <div class="owner-class-capacity"><strong>${registered === null ? "—" : registered}${Number.isFinite(capacity) ? ` / ${capacity}` : ""}</strong><span>${registered === null ? "Registration tracking not connected" : "registered"}</span></div>
      <span class="owner-status ${item.published === false ? "is-draft" : ""}">${item.published === false ? "Draft" : "Published"}</span>
      <div class="owner-class-actions">
        <a href="/${encodeURIComponent(business.slug)}?owner=1&edit=classes&class=${index}">Edit</a>
        <button type="button" data-class-action="duplicate" data-class-index="${index}">Duplicate</button>
        <button type="button" data-class-action="toggle" data-class-index="${index}">${item.published === false ? "Publish" : "Unpublish"}</button>
        <button type="button" data-class-action="delete" data-class-index="${index}">Delete</button>
      </div>
    </article>`;
  };
  const render = () => {
    const draftClasses = classesFor(draftState);
    const liveClasses = classesFor(publishedState).filter((item) => item.published !== false);
    const next = liveClasses[0];
    root.innerHTML = `<div class="owner-shell">
      <nav class="owner-nav" aria-label="Dashboard navigation">
        <a class="is-active" href="/dashboard/">Overview</a><a href="#classes">Classes</a><a href="#registrations">Registrations</a><a href="/${encodeURIComponent(business.slug)}?owner=1&edit=classes">Website</a><a href="#analytics">Analytics <span>PRO</span></a><a href="#settings">Settings</a>
      </nav>
      <section class="owner-welcome">
        <div><p class="eyebrow">Owner dashboard</p><h1>Welcome back, ${esc(displayName(user))}.</h1><p>Here’s what’s happening with your website and classes.</p></div>
        <div class="owner-primary-actions"><a class="primary-button" href="/${encodeURIComponent(business.slug)}?owner=1&edit=classes">Edit Website</a><a class="secondary-button" href="/${encodeURIComponent(business.slug)}" target="_blank" rel="noopener">View Public Site</a><a class="secondary-button" href="/${encodeURIComponent(business.slug)}?owner=1&edit=classes&new=1">Add Class</a></div>
      </section>
      ...
      <section class="owner-metrics" aria-label="Business summary">
        <article><span>Live Classes</span><strong>${liveClasses.length}</strong><p>${liveClasses.length ? "Currently visible on your website" : "Add your first class to begin"}</p><a href="#classes">Manage classes</a></article>
        <article><span>Student Registrations</span><strong>—</strong><p>Registration data will appear after booking is connected.</p><a href="#registrations">View registrations</a></article>
        <article><span>Upcoming Session</span><strong>${esc(next?.date || "—")}</strong><p>${next ? `${esc(next.title)} • ${esc(next.time || "Time TBA")}` : "No upcoming sessions published"}</p><a href="#classes">View class</a></article>
        <article class="is-pro"><span>Website Views <b>PRO</b></span><strong>—</strong><p>Unlock visitor analytics and traffic insights.</p><a href="#analytics">See Analytics Upgrade</a></article>
      </section>
      ...
      <section id="classes" class="owner-dashboard-section"><header><div><p class="eyebrow">Schedule</p><h2>Upcoming Classes</h2></div><a class="primary-button" href="/${encodeURIComponent(business.slug)}?owner=1&edit=classes&new=1">+ Add Class</a></header>
        <div class="owner-class-list">${draftClasses.length ? draftClasses.map(classRow).join("") : `<div class="owner-empty-state"><strong>Your website is live. Now add your first class.</strong><p>Classes you create will appear here and on your public website after publishing.</p><a class="primary-button" href="/${encodeURIComponent(business.slug)}?owner=1&edit=classes&new=1">Add a Class</a></div>`}</div>
      </section>
  ...
  const saveDraft = async (message) => {
    saving = true; render();
    try { await app.saveWebsiteDraft({ user, businessId: business.id, state: draftState }); }
    catch (error) { window.alert(error.message || message || "Draft could not be saved."); }
  ...
  const bindEvents = () => {
    ...
    root.querySelectorAll("[data-class-action]").forEach((button) => button.addEventListener("click", async () => {
      const index = Number(button.dataset.classIndex); const action = button.dataset.classAction; const classes = classesFor(draftState);
      if (action === "duplicate") classes.splice(index + 1, 0, { ...clone(classes[index]), title: `${classes[index].title} Copy`, published: false });
      if (action === "toggle") classes[index].published = classes[index].published === false;
      if (action === "delete" && window.confirm(`Delete ${classes[index].title}?`)) classes.splice(index, 1);
      else if (action === "delete") return;
      draftState.classes = classes; await saveDraft("Class change could not be saved.");
    }));
  };
  try {
    if (!app?.client || !templates) throw new Error("BeyondEight services are unavailable.");
    ...
    business = businesses.find((item) => item.id === requested) || businesses[0] || null;
    ...
    bundle = await app.getBusinessBundle(business.id);
    draftState = sourceState("draft"); publishedState = sourceState("published"); render();
