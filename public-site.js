(async function () {
  ...
  const updateStateFromForm = (form) => {
    ...
    if (form.dataset.editorSection === "classes") {
      ...
      state.classes = [...form.querySelectorAll("[data-class-index]")].map((group, index) => {
        const values = Object.fromEntries([...group.querySelectorAll("input:not([type=file]), textarea, select")].map((input) => [input.name, input.value]));
        return { ...previous[index], ...values, published: values.published !== "Draft" };
      });
  ...
  const openEditor = (section, focus = true) => {
    ...
    const editorGroups = [
      ["Content", [["classes", "Classes"], ["workshop", "Workshops"], ["about", "Instructor / About"], ["gallery", "Images & Gallery"], ["social", "Instagram / Social"], ["contact", "Contact"]]],
      ["Design", [["theme", "Theme & Brand"], ["hero", "Header & Hero"], ["footer", "Footer"]]],
  ...
  try {
    ...
    if (ownsPublicBundle && (params.get("owner") === "1" || requestedEditor)) {
      editMode = true;
      if (requestedEditor === "classes" && params.get("new") === "1") {
        state.classes = [...(state.classes || contentForState().classes), { title: "New Class", style: "Open", date: "Coming soon", time: "TBA", duration: "60 minutes", location: "In studio", format: "In person", level: "Open level", price: "$25", capacity: "20", spots: "20 spots left", instructor: state.instructorName || contentForState().instructorName, published: false }];
        dirty = true;
        scheduleSave();
      }
    }
