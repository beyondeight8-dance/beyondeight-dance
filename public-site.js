(async function () {
  ...
  const editorBody = (section) => {
    ...
    return ({
      ...
      benefits: `${field("Section label", "benefitsEyebrow", state.benefitsEyebrow ?? content.benefitsEyebrow)}${field("Section heading", "benefitsHeading", state.benefitsHeading ?? content.benefitsHeading)}${area("Benefits (Title | Description, one per line)", "benefits", (state.benefits || content.benefits).map((item) => item.join(" | ")).join("\n"))}`,
      ...
      faq: `${field("Section label", "faqEyebrow", state.faqEyebrow ?? content.faqEyebrow)}${field("Section heading", "faqHeading", state.faqHeading ?? content.faqHeading)}${area("FAQ (Question | Answer, one per line)", "faqs", (state.faqs || content.faqs).map((item) => item.join(" | ")).join("\n"))}`,
  ...
  const updateStateFromForm = (form) => {
    ...
    if (form.dataset.editorSection === "classes") {
      ...
      // update just that one entry by its index and leave every other class untouched.
      const previous = state.classes || contentForState().classes;
      const expandedGroup = form.querySelector("[data-class-index]");
      if (expandedGroup) {
        const idx = Number(expandedGroup.dataset.classIndex);
        // published is set only by the explicit Save as Draft / Publish buttons below, never by
        // a field in this scan - there is no "published" input here to accidentally pick up.
        const values = Object.fromEntries([...expandedGroup.querySelectorAll("input:not([type=file]), textarea, select")].map((input) => [input.name, input.value]));
        state.classes = previous.map((item, index) => index === idx ? { ...item, ...values } : item);
      } else {
        state.classes = previous;
      }
      form.querySelectorAll(".owner-section-intro-fields input, .owner-section-intro-fields textarea").forEach((input) => { state[input.name] = input.value; });
      ...
      else if (key === "testimonials" || key === "faqs" || key === "benefits") state[key] = String(value).split(/\n+/).map((line) => line.split("|").map((part) => part.trim())).filter((item) => item[0] && item[1]);
